...
const count = (target, input) => {
  return input.reduce((accumulator, outerArr) => {
    let innerArr = outerArr.reduce((innerValue, innerPointer) => {
      if (innerPointer === target) {
        return innerValue + 1;
      }
      return innerValue;
    }, 0);
...
let findShortest = (data) => {
  data.sort((a, b) => {
    if (a.starWarsData.height > b.starWarsData.height) {
      return 1;
    }
    else if (a.starWarsData.height < b.starWarsData.height) {
      return -1;
    }
    else {
      return a.starWarsData.height > b.starWarsData.height ? 1 : -1;
    }

  });
  return data[0];
  // data.map(starWarsData.height);
  // if ()
};
